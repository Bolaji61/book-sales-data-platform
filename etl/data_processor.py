...
from models import (AnalyticsOverview, DailySalesSummary, TopBooksSummary,
...
class DataProcessor:
    ...
    def __init__(self):
        ...
        self.redshift_cluster = os.getenv(
        ...
        self.redshift_client = boto3.client(
            "redshift-data", region_name=self.aws_region
        )
    ...
    async def _load_and_clean_data(self):
        ...
        log_info(
            f"Loaded {len(self.users_df)} users, {len(self.transactions_df)} transactions, {len(self.books_df)} books"
        )
    ...
    def _clean_users(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        ).filter(pl.col("signup_date").is_not_null() & pl.col("id").is_not_null())
    ...
    async def _upload_and_load_data(self):
        ...
        users_df_copy = (
            self.users_df.rename({"id": "user_id"})
            .with_columns(
                [
    ...
    async def _upload_csv_to_s3(self, df: pl.DataFrame, s3_key: str):
        ...
        try:
            ...
            self.s3_client.put_object(
                Bucket=self.s3_bucket, Key=s3_key, Body=csv_data, ContentType="text/csv"
            )
    ...
    async def get_top_books_summary(self, limit: int = 10) -> List[TopBooksSummary]:
        ...
        return [
            TopBooksSummary(
                book_id=1, title="Sample Book", total_revenue=500.0, total_sales=25
            )
        ]
